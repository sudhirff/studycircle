    ...
    setup(props, context) {
        ...
        const fetch = async() => {
            ...
            try {
                ...
                const result = await axios.get(`/api/v1/roles/${id}/edit`);
                if (result.status != 200) {
                    const error = new Error('Failed to fetch roles')
                    throw error;
                }
                role.name = result.data.response.name;
                role.permissions = result.data.response.oldpermissions;
                role.id = result.data.response.id;
            } catch (e) {
        ...
        async function submitForm() {
            ...
            if (!v$.value.$error) {
                isLoading.value = true;
                try {
                    await store.dispatch('roles/updateRole', role);
                    isLoading.value = false;
