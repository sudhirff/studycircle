<template>
    <div>
        <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
            <h2 class="text-lg font-medium mr-auto">{{ $t('Edit Chapter')}}</h2>
...
export default {
    ...
    setup(props, context) {
        ...
        editMode.value = true;
        // After items are fetched, we have to get all the subjects using gettters
        ...
        const fetch = async() => {
            ...
            try {
                ...
                const result = await axios.get(`/api/v1/chapters/${id}/edit`);
                ...
                form.parent_id = response.parent_id;
                form.label = JSON.parse(response.label);
                form.description = JSON.parse(response.description);
                form.icon = response.icon;
                form.tags = response.tags;
                form.language_id = response.language_id;
                form.tags = response.tagged.map((tag) => { return tag.tag_name});
            } catch (e) {
